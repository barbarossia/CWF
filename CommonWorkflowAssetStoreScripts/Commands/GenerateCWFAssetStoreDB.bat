...
if %ERRORLEVEL% NEQ 0 goto errors
echo "   **** Created sproc WorkflowType_Search.sql"
...
if %ERRORLEVEL% NEQ 0 goto errors
echo "   **** Created sproc TaskActivity_CreateOrUpdate.sql"

sqlcmd -S %server% -d %database% -E -b -i "TaskActivity_Get.sql"
if %ERRORLEVEL% NEQ 0 goto errors
echo "   **** Created sproc TaskActivity_Get.sql"

sqlcmd -S %server% -d %database% -E -b -i "TaskActivity_Search.sql"
if %ERRORLEVEL% NEQ 0 goto errors
echo "   **** Created sproc TaskActivity_Search.sql"

sqlcmd -S %server% -d %database% -E -b -i "TaskActivity_UpdateStatus.sql"
if %ERRORLEVEL% NEQ 0 goto errors
echo "   **** Created sproc TaskActivity_UpdateStatus.sql"
