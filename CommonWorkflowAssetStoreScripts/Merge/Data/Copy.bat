...
sqlcmd -S %server% -d %database% -E -b -i "ActivityLibrary.sql" -v DBName = '%olddb%' Evr = '%env%'
if %ERRORLEVEL% NEQ 0 goto errors
...
sqlcmd -S %server% -d %database% -E -b -i "Activity.sql" -v DBName = '%olddb%' Evr = '%env%'
if %ERRORLEVEL% NEQ 0 goto errors
...
sqlcmd -S %server% -d %database% -E -b -i "WorkflowType.sql" -v DBName = '%olddb%' Evr = '%env%'
if %ERRORLEVEL% NEQ 0 goto errors
echo "   ***** Merged table WorkflowType

sqlcmd -S %server% -d %database% -E -b -i "UpdateWorkflowType.sql" -v DBName = '%olddb%' Evr = '%env%'
if %ERRORLEVEL% NEQ 0 goto errors
